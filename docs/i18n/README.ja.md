...
**システム設定ではオンになっているのに、GhostTypeが「権限がない」と言う。**
0.3.1以前から更新した人は全員これに当たります。macOSはアクセシビリティの許可を、それを受け取ったアプリのコード署名に結びつけて保存します。0.3.1までの各リリースはビルドごとに変わるハッシュで署名されていたため、バージョンが変わるたびに署名も変わっていました。1.0.0からは固定の証明書で署名するのでここで打ち止めですが、あなたのMacには古い規則が残ったままです。設定画面のスイッチは許可の値を変えるだけで、そこに紐づく規則を書き直しません。だからスイッチはオンに見えるのに、アプリは拒否され続けます。

スイッチをオフにしてオンに戻しても消えません。マイナスボタンで一覧からGhostTypeを削除しても消えません。保存された項目そのものを削除する必要があります。
...
そのあとGhostTypeを起動し直し、求められたら承認してください。必要なのは一度だけです。自分がこの症状かどうかは、次のログに`Failed to match existing code requirement`が出るかで判別できます。
...
**他のアプリで補完が出ない。**
メニューバーの状態を見てください。「アクセシビリティを許可」または「入力監視を許可」と出ていたら、システム設定の該当ペインでGhostTypeをオンにします。数秒で自動的に再起動します。ソースからビルドすると署名し直されるため、署名IDを指定せずにビルドすると毎回署名が変わり、ビルドのたびにアクセシビリティの再許可を求められます。`scripts/make-signing-cert.sh`を一度実行し、`GHOSTTYPE_SIGN_IDENTITY`を設定してビルドすれば、再ビルドしても許可が残ります。
